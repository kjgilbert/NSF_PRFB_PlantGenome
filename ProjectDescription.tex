...
Differences between simulations and observed data are informative for the history of selection on phenotypes, and I will again use fwdpy and an ABC approach to estimate the strength of selection on domestication phenotypes. %\jri{i don't think this has ever been done except backwards from archaeological data!} 
...
%\kjg{or if interact w/ demog in weird ways, then could change in unexpected ways? - i.e. if demog sends in one direction but seln senda back in the other , could cancel out instead of build up}
 than those not under directional selection passing through the bottleneck and I can test if allelic effect sizes have shifted in the distribution's mean or overall shape to more or fewer loci of large effect \citep{Chevin:2008}. %chevin = effect of directional selection on different QTL allele sizes} 
